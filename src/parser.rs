use crate::{dato::Datos, error::ErrorType, lexer::Operador, queries::{comparadores::{ExpresionBooleana, OperadorComparacion}, DeleteQuery, InsertQuery, SQLQuery, SelectQuery, UpdateQuery}};
...
// Extrae el String más interno de la lista si no tiene más de un elemento
fn extraer_interno_lista_string(operador: &[Operador]) -> Result<String, ErrorType> {
    if operador.len() != 1 {
        return Err(ErrorType::InvalidSyntax("Cantidad de elementos incorrecta".to_string()));
    }
    match &operador[0] {
        Operador::String(s) |  Operador::Texto(s)=> Ok(s.to_string()),
        Operador::Lista(operador) => extraer_interno_lista_string(operador.as_slice()),
        Operador::Comparador(_) => Err(ErrorType::InvalidSyntax("Comparador inesperado.".to_string())),
    }
}

// Recibe cualquier operador y lo convierte en un String
fn operador_to_single_string(operador: &Operador) -> Result<String, ErrorType> {
    match operador {
        Operador::String(s) | Operador::Texto(s) => Ok(s.to_string()),
        Operador::Lista(list) => extraer_interno_lista_string(list.as_slice()),
        Operador::Comparador(_) => Err(ErrorType::InvalidSyntax("Comparador inesperado.".to_string())),
    }
}
...
fn operador_to_value(lista: &Vec<Operador>) -> Result<Vec<Datos>, ErrorType> {
    let mut datos: Vec<Datos> = Vec::new();
    for operador in lista {
        let item = operador_to_dato(operador)?; // aca era operador_to_single_dato?? Estoy seguro pero por las dudas l odejo
        datos.push(item);
    }
    Ok(datos)
}
...
// ["value1", "value2", "..."] ["value1", "value2", "..."] ... a Vec<Vec<Datos>
fn operador_to_values(lista: &[Operador]) -> Result<Vec<Vec<Datos>>, ErrorType>{
    let mut values: Vec<Vec<Datos>> = Vec::new();
    for operador in lista{
        match operador {
            Operador::Lista(operador_items) => values.push(operador_to_value(operador_items)?),
...
fn crear_comparacion(izq: &Operador, comparador: &String, der: &Operador) -> Result<ExpresionBooleana, ErrorType> {
    Ok(ExpresionBooleana::Comparacion {
        izq: operador_to_single_string(izq)?, 
        operador: string_to_comparacion(comparador)?,
        der: operador_to_single_string(der)?
    })
}
...
fn parser_insert(table: &String, columns: &Vec<Operador>, values: &[Operador])-> Result<InsertQuery, ErrorType>{
    ...
    let values_parsed: Vec<Vec<Datos>> = operador_to_values(values)?;
    for value in &values_parsed {
        if value.len() != columns_parsed.len() {
            return Err(ErrorType::InvalidSyntax("Todos los valores deben tener el mismo numero de terminos".to_string()));
        }
    }
    Ok(InsertQuery::new(table, columns_parsed, values_parsed))
}
...
pub fn parser(query: Vec<Operador>) -> Result<SQLQuery, ErrorType>{
    match query.as_slice(){ // [Operacion, tabla, ...]
        ...
        _ => Err(ErrorType::InvalidSyntax("Query invalida".to_string())), // operacion invalida
    }
}